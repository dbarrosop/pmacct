...
inline static void putClass(SFLReceiver *receiver, SFLExtended_classification *class_elem)
{
  char buf[MAX_PROTOCOL_LEN+1];

  memset(buf, 0, MAX_PROTOCOL_LEN+1);
  if (class_elem->class && class[class_elem->class-1].id) {
    strlcpy(buf, class[class_elem->class-1].protocol, MAX_PROTOCOL_LEN);
    buf[sizeof(buf)-1] = '\0';
  }
...
inline static void putTag(SFLReceiver *receiver, SFLExtended_tag *tag_elem)
{
  ...
  putNet64(receiver, tag_elem->tag2);
}
